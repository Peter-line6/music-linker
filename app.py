...
# Les clés d'API ne sont plus nécessaires pour la recherche, mais on peut les garder pour plus tard
    ...
    try:
        ...
        for platform_name, platform_data in data['linksByPlatform'].items():
            if platform_name in platform_map:
                variable_name = platform_map[platform_name]
                resultats[variable_name] = platform_data.get('url')
...
def page_edition(slug):
    ...
    resultats = {
        'item_type': page_a_editer.item_type,
        ...
        'steam_description': page_a_editer.steam_description,
